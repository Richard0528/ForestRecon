...
Fit_cal_func.fn<-function(ncol=4, nrow=10, sim_section=sim.density.section, 
                          est_section=est.density.section.bia, nsim=4000, fit_cal="MSE",bySection=TRUE)
{
  ...
  for (i in 1:10)
  {
    ...
    {
      if (fit_cal == "MSE"){
        cur_matrix.df[i,1] = sum((est_section[,i] - sim.tph.all)^2)/nsim
      } else if (fit_cal == "RME"){
        cur_matrix.df[i,1] = sum((est_section[,i] - sim.tph.all)/sim.tph.all)/nsim
      } else if (fit_cal == "RMAE"){
        cur_matrix.df[i,1] = sum(abs(est_section[,i] - sim.tph.all)/sim.tph.all)/nsim
      }
 #     colnames(cur_matrix.df) = c("section1", "section2", "section3", "section4")
